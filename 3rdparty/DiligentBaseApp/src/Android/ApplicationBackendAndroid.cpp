...
class ApplicationBackendAndroid final : public ApplicationBackend
{
public:
    ApplicationBackendAndroid()
    {
    ...
    virtual void DrawFrame() override final
    {
        ...
        if (m_WindowWidth != SCDesc.Width || m_WindowHeight != SCDesc.Height || m_PreTransform != SCDesc.PreTransform)
        {
            ...
            m_TheApplication->WindowResize(static_cast<int>(m_WindowWidth), static_cast<int>(m_WindowHeight));
        }

        ApplicationBackend::DrawFrame();
    }
    ...
    virtual int Resume(ANativeWindow* window) override final
    {
        switch (m_DeviceType)
        {
            ...
            case RENDER_DEVICE_TYPE_VULKAN:
            {
                ...
                m_TheApplication->ResetSwapChain(nullptr);
                m_pDevice->IdleGPU();
                ...
                GetEngineFactoryVk()->CreateSwapChainVk(m_pDevice, GetImmediateContext(),
                                                        ...
                                                        &m_pSwapChain);
                m_TheApplication->ResetSwapChain(m_pSwapChain);
        ...
        if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION)
        {
            ...
            else
            {
                ...
                if (dragState & ndk_helper::GESTURE_STATE_START)
                {
                    ...
                    if (!Handled)
                    {
                        m_TheApplication->GetInputController().BeginDrag(fX, fY);
                    }
                }
                else if (dragState & ndk_helper::GESTURE_STATE_MOVE)
                {
                    ...
                    if (!Handled)
                    {
                        m_TheApplication->GetInputController().DragMove(fX, fY);
                    }
                }
                else if (dragState & ndk_helper::GESTURE_STATE_END)
                {
                    static_cast<ImGuiImplAndroid*>(m_pImGui.get())->EndDrag();
                    m_TheApplication->GetInputController().EndDrag();
                }
                ...
                if (pinchState & ndk_helper::GESTURE_STATE_START)
                {
                    ...
                    v1.Value(fX1, fY1);
                    v2.Value(fX2, fY2);
                    m_TheApplication->GetInputController().StartPinch(fX1, fY1, fX2, fY2);
                ...
                else if (pinchState & ndk_helper::GESTURE_STATE_MOVE)
                {
                    ...
                    v1.Value(fX1, fY1);
                    v2.Value(fX2, fY2);
                    m_TheApplication->GetInputController().PinchMove(fX1, fY1, fX2, fY2);
                    //tap_camera_.Pinch( v1, v2 );
                }
                else if (pinchState & ndk_helper::GESTURE_STATE_END)
                {
                    m_TheApplication->GetInputController().EndPinch();
                }
            }
            return 1;
        }
...
{
    return new ApplicationBackendAndroid;
}
