...
class ApplicationBackendMacOS final : public ApplicationBackend
{
public:
    ApplicationBackendMacOS()
    {
    ...
    {
        std::lock_guard<std::mutex> lock(AppMutex);
        ApplicationBackend::Update(CurrTime, ElapsedTime);
    }

    virtual void WindowResize(int width, int height)override
    {
        std::lock_guard<std::mutex> lock(AppMutex);
        ApplicationBackend::WindowResize(width, height);
    }

    virtual void Present()override
    {
        std::lock_guard<std::mutex> lock(AppMutex);
        ApplicationBackend::Present();
    }
    ...
    virtual void HandleOSXEvent(void* _event, void* _view)override final
    {
        ...
        std::lock_guard<std::mutex> lock(AppMutex);
        auto& inputController = m_TheApplication->GetInputController();
...
{
    return new ApplicationBackendMacOS;
}
