...
class ApplicationBackendLinux final : public ApplicationBackend
{
public:
    ApplicationBackendLinux()
    {
    ...
    virtual int HandleXEvent(XEvent* xev) override final
    {
        ...
        {
            handled = m_TheApplication->GetInputController().HandleXEvent(xev);
        }
        return handled;
    }
    ...
    virtual bool InitVulkan(xcb_connection_t* connection, uint32_t window) override final
    {
        try
        {
            ...
            m_TheApplication->GetInputController().InitXCBKeysms(connection);
            InitializeSample();
            return true;
        }
        catch (...)
    ...
    virtual void HandleXCBEvent(xcb_generic_event_t* event) override final
    {
        ...
        {
            handled = m_TheApplication->GetInputController().HandleXCBEvent(event);
        }
    }
#endif
};
...
{
    return new ApplicationBackendLinux;
}
